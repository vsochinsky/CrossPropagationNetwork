...
int sensor_layer::ImageInit(char *FileName){
	...
	if ((pict.Image = (unsigned char *) malloc(sizeof(unsigned char)*3*pict.W * pict.H)) == NULL){
		//MessageBox(NULL, L"error reading image", NULL, MB_OK); //TODO
		fclose(F);
		return 0;
	}
...
sensor_layer::sensor_layer(){
	...
	if(!ImageInit("./sensors/cover_hght.bmp")){
		//MessageBox(NULL, L"cannot open image", NULL, MB_OK); //TODO
		return;
	}


	while(fscanf(in, "%d %d %d %d %d\n", &xc, &yc, &w, &h, &N)!=EOF){ //считываем сенсоры
		PascalCoord2Normal(xc, yc, w, h, &x1, &y1, &x2, &y2);
		if(i==0){
			i=1;
			Head=S;
		}
...
void sensor_layer::PascalCoord2Normal(int xc, int yc, int w, int h, int *x1, int*y1, int*x2, int*y2){
	*x1=xc+w/2;
	*y1=yc+h/2;
	*x2=xc-w/2;
	*y2=yc-h/2;
